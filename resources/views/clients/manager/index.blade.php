    ...
    <x-slot name="header">
        <div class="relative" x-data="{ open: false }">
            <form method="GET" action="{{ route('clients.index') }}">
                <div class="flex flex-wrap items-center justify-between gap-4">
                    ...
                    <div class="flex items-center gap-2">
                        <input type="text" name="search" placeholder="Buscar..."
                            class="w-48 rounded-md shadow-sm border-gray-300 text-sm focus:ring-indigo-500 focus:border-indigo-500"
                ...
                <div x-show="open"
                     x-transition
                     class="absolute top-full right-0 w-full md:w-auto mt-2 z-20"
...
<tbody class="bg-white divide-y divide-gray-200">
    @forelse ($clients as $client)
        <tr>
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {{-- Muestra la ciudad de la primera dirección de servicio, o 'N/A' si no tiene --}}
                {{ $client->serviceAddresses->first()->ciudad ?? 'N/A' }}
            </td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {{-- Muestra el estado del contrato más reciente, o 'Sin Contrato' si no tiene --}}
                @php
