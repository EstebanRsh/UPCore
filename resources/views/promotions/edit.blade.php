<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Editando Promoción: {{ $promotion->nombre }}
        </h2>
    </x-slot>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input id="nombre" name="nombre" type="text"
                                    value="{{ old('nombre', $promotion->nombre) }}" class="block mt-1 w-full"
                                    required />
                            </div>
                            <div>
                                <label for="duracion_meses">Duración (en meses)</label>
                                <input id="duracion_meses" name="duracion_meses" type="number"
                                    value="{{ old('duracion_meses', $promotion->duracion_meses) }}"
                            ...
                            <div>
                                ...
                                <select id="tipo_descuento" name="tipo_descuento" class="block mt-1 w-full" required>
                                    <option value="porcentaje"
                                        {{ $promotion->tipo_descuento == 'porcentaje' ? 'selected' : '' }}>Porcentaje
                                        (%)</option>
                                    <option value="monto_fijo"
                                        {{ $promotion->tipo_descuento == 'monto_fijo' ? 'selected' : '' }}>Monto Fijo
                                        ($)</option>
                                    <option value="meses_gratis"
                                        {{ $promotion->tipo_descuento == 'meses_gratis' ? 'selected' : '' }}>Meses
                                        Gratis</option>
                                </select>
                            </div>
                            <div>
                                ...
                                <input id="valor_descuento" name="valor_descuento" type="number" step="0.01"
                                    value="{{ old('valor_descuento', $promotion->valor_descuento) }}"
                                    class="block mt-1 w-full" required />
                            </div>
                        </div>
                        <div class="mt-4">
                            <label for="descripcion">Descripción</label>
                            <textarea id="descripcion" name="descripcion" rows="4" class="block mt-1 w-full" required>{{ old('descripcion', $promotion->descripcion) }}</textarea>
                        </div>
